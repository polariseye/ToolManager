...
namespace Plugn.CodeGenerate.T4TemplateGenerate
{
    ...
    /// 多表生成
    /// </summary>
    public class MultTableGenerate : GenerateBase
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        {
            return this.TemplateInfoList.Count;
        }
        ...
        public override Boolean Generate()
        {
            ...
            foreach (var templateItem in this.TemplateInfoList)
            {
                ...
                var errMsg = BuildTable(this.TableList, templateItem,out fileName);
        ...
        /// 生成一个表
        /// </summary>
        /// <param name="tableList">表对象</param>
        /// <param name="templateItem">模板列表</param>
        /// <param name="savedFileName">保存到的文件名</param>
        /// <param name="paramData">参数数据</param>
        private String BuildTable(List<SOTable> tableList, TemplateInfo templateItem,out String savedFileName)
        {
            //生成代码文件
            MultTableHost host = new MultTableHost(this.NameRuleConfig, this.TypeMapConfigList, tableList);
